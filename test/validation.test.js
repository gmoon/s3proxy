...
describe('S3Proxy Validation Tests', () => {
  ...
  describe('Basic File Serving', () => {
    test('should serve HTML file with correct headers', async () => {
      ...
      expect(parseInt(response.headers.get('content-length'))).toBe(EXPECTED_FILES['/index.html'].contentLength);
    ...
    test('should serve zero-byte file correctly', async () => {
      ...
      expect(response.headers.get('content-type')).toBe(EXPECTED_FILES['/zerobytefile'].contentType);
      expect(parseInt(response.headers.get('content-length'))).toBe(EXPECTED_FILES['/zerobytefile'].contentLength);
  ...
  describe('Binary File Integrity', () => {
    test('should serve large binary file without corruption', async () => {
      ...
      expect(parseInt(response.headers.get('content-length'))).toBe(EXPECTED_FILES['/large.bin'].contentLength);
    ...
    test('should serve medium binary file correctly', async () => {
      ...
      expect(parseInt(response.headers.get('content-length'))).toBe(EXPECTED_FILES['/test1m.tmp'].contentLength);
  ...
  describe('Range Requests (Partial Content)', () => {
    test('should handle simple range request', async () => {
      ...
      expect(parseInt(response.headers.get('content-length'))).toBe(100);
    ...
    test('should handle range request in middle of file', async () => {
      ...
      expect(parseInt(response.headers.get('content-length'))).toBe(1000);
    ...
    test('should handle range request to end of file', async () => {
      ...
      expect(response.status).toBe(206);
      expect(parseInt(response.headers.get('content-length'))).toBe(60);
  ...
  describe('HEAD Requests', () => {
    test('should handle HEAD request for HTML file', async () => {
      ...
      expect(parseInt(response.headers.get('content-length'))).toBe(338);
    ...
    test('should handle HEAD request for binary file', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.headers.get('content-type')).toBe('application/octet-stream');
      expect(parseInt(response.headers.get('content-length'))).toBe(10485760);
  ...
  describe('Performance and Reliability', () => {
    ...
    test('should handle large file download within reasonable time', async () => {
